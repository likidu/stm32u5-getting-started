# Using Visual Studio with the STMicroelectronics B-U585I-IOT02A Discovery Kit

This guide describes how to use Visual Studio to build, flash, and debug with the **STMicroelectronics B-U585I-IOT02A Discovery Kit**.
...
1. Extract `tools\openocd-st.zip` to somewhere and add the path to your **Path** of the Windows system environment variables.

    > We don't use the vcpkg to install OpenOCD because so far the official OpenOCD repo does not provide the support for STM32U5 series. You need to use ST customized fork: https://github.com/STMicroelectronics/OpenOCD. The zip file is a precompiled version for Windows from this repo.

1. Run the following code to confirm that OpenOCD version 0.11 is installed.
...
1. Ensure that both the Configure Preset and the Build Preset are set to "arm-gcc-cortex-m4"
    ...
    ![configuration-choice](../../docs/media/vs-preset-configuration-m33.png)

1. Ensure that the "Launch" target is selected as the Debug Target.

    ![debug-target-choice](../../docs/media/vs-debug-target-stm32u5.png)
