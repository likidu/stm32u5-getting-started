...
static UINT sntp_client_run()
{
    ...
    //if ((status = nx_sntp_client_initialize_unicast(&sntp_client, SAMPLE_SNTP_SERVER_ADDRESS)))
    if ((status = nxd_sntp_client_initialize_unicast(&sntp_client, &sntp_address)))
    {
